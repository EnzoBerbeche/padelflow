...
export interface ClubStatistics {
  ...
  most_active_male_player: {
    licence: string;
    nom: string | null;
    nb_tournois: number | null;
  };
  most_active_female_player: {
    licence: string;
    nom: string | null;
    nb_tournois: number | null;
  };
  most_efficient_male_player: {
    licence: string;
    nom: string | null;
    points_per_tournament: number;
  };
  most_efficient_female_player: {
    licence: string;
    nom: string | null;
    points_per_tournament: number;
  };
  monthly_evolution: Array<{
    ...
    male_points: number;
    female_points: number;
  }>;
}

export const clubsAPI = {
  // Get all unique clubs from latest rankings
  getAllClubs: async (): Promise<string[]> => {
    ...
    const uniqueClubs = Array.from(new Set(data?.map(row => row.club).filter(Boolean)));
    return uniqueClubs.sort();
  },

  // Get club statistics
  getClubStatistics: async (clubName: string): Promise<ClubStatistics | null> => {
    try {
      // Get all players from the club using latest rankings view
      ...
      if (rankingsError) {
        console.error('Error fetching club rankings:', rankingsError);
        return null;
      }
      ...
      // Since we're using rankings_latest, we already have the latest ranking per player
      const players = rankingsData;
      
      // Calculate basic statistics
      const total_players = players.length;
      const male_players = players.filter(p => p.genre === 'Homme').length;
      const female_players = players.filter(p => p.genre === 'Femme').length;
      
      // Calculate average age
      const validBirthYears = players
        .map(p => p.annee_naissance)
        .filter((year): year is number => year !== null && year > 1900);
      const currentYear = new Date().getFullYear();
      const average_age = validBirthYears.length > 0 
        ? Math.round(validBirthYears.reduce((sum, year) => sum + (currentYear - year), 0) / validBirthYears.length)
        : 0;
      ...
      players.forEach(p => {
        if (p.nationalite) {
          nationalityCounts.set(p.nationalite, (nationalityCounts.get(p.nationalite) || 0) + 1);
        }
      });
      const top_nationalities = Array.from(nationalityCounts.entries())
        .map(([nationality, count]) => ({ nationality, count }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 3);

      // Get top 10 players by gender
      const top_10_male = players
        .filter(p => p.genre === 'Homme' && p.rang)
        ...
        .map(p => ({
          ...
          meilleur_classement: p.meilleur_classement,
        }));

      const top_10_female = players
        .filter(p => p.genre === 'Femme' && p.rang)
        .sort((a, b) => (a.rang || 0) - (b.rang || 0))
        .slice(0, 10)
        .map(p => ({
          licence: p.licence,
      ...
      const validRankings = players.map(p => p.rang).filter((r): r is number => r !== null);
      const average_ranking = validRankings.length > 0 
        ? Math.round(validRankings.reduce((sum, r) => sum + r, 0) / validRankings.length)
        : 0;

      const top_10_count = players.filter(p => p.rang && p.rang <= 10).length;
      const top_50_count = players.filter(p => p.rang && p.rang <= 50).length;
      const top_100_count = players.filter(p => p.rang && p.rang <= 100).length;

      // Find most active male player
      const most_active_male_player = players
        .filter(p => p.genre === 'Homme' && p.nb_tournois && p.nb_tournois > 0)
        .sort((a, b) => (b.nb_tournois || 0) - (a.nb_tournois || 0))[0] || {
          licence: '',
          nom: null,
          nb_tournois: null,
        };

      // Find most active female player
      const most_active_female_player = players
        .filter(p => p.genre === 'Femme' && p.nb_tournois && p.nb_tournois > 0)
        .sort((a, b) => (b.nb_tournois || 0) - (a.nb_tournois || 0))[0] || {
          licence: '',
          nom: null,
          nb_tournois: null,
      ...
      // Find most efficient male player (points per tournament)
      const most_efficient_male_player = players
        .filter(p => p.genre === 'Homme' && p.points && p.nb_tournois && p.nb_tournois > 0)
        .map(p => ({
          licence: p.licence,
          nom: p.nom,
          points_per_tournament: Math.round((p.points || 0) / (p.nb_tournois || 1)),
        }))
        .sort((a, b) => b.points_per_tournament - a.points_per_tournament)[0] || {
          licence: '',
          nom: null,
          points_per_tournament: 0,
        };

      // Find most efficient female player (points per tournament)
      const most_efficient_female_player = players
        .filter(p => p.genre === 'Femme' && p.points && p.nb_tournois && p.nb_tournois > 0)
        .map(p => ({
          licence: p.licence,
          nom: p.nom,
          points_per_tournament: Math.round((p.points || 0) / (p.nb_tournois || 1)),
      ...
      const monthly_evolution = await clubsAPI.getMonthlyEvolution(clubName);
  ...
  // Get monthly evolution for the last 12 months
  getMonthlyEvolution: async (clubName: string): Promise<ClubStatistics['monthly_evolution']> => {
    try {
      // First, find the latest month with data in the database
      const { data: latestData, error: latestError } = await supabase
        .from('rankings')
        .select('ranking_year, ranking_month')
        .order('ranking_year', { ascending: false })
        .order('ranking_month', { ascending: false })
        .limit(1);

      if (latestError || !latestData || latestData.length === 0) {
        console.error('Error finding latest month:', latestError);
        return [];
      }

      const latestYear = latestData[0].ranking_year;
      const latestMonth = latestData[0].ranking_month;

      // Get data for the last 12 months from the latest month found
      const monthlyData: ClubStatistics['monthly_evolution'] = [];
      
      for (let i = 11; i >= 0; i--) {
        let targetMonth = latestMonth - i;
        ...
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('club', clubName)
          .eq('ranking_year', targetYear)
          .eq('ranking_month', targetMonth);

        if (error) {
          console.error(`Error fetching data for ${targetYear}-${targetMonth}:`, error);
          continue;
        }

        if (data && data.length > 0) {
          const total_points = data.reduce((sum, row) => sum + (row.points || 0), 0);
          const total_tournaments = data.reduce((sum, row) => sum + (row.nb_tournois || 0), 0);
          const active_players = data.length;
          const male_points = data
            .filter(row => row.genre === 'Homme')
            .reduce((sum, row) => sum + (row.points || 0), 0);
          const female_points = data
            .filter(row => row.genre === 'Femme')
            .reduce((sum, row) => sum + (row.points || 0), 0);
      ...
      console.error('Error getting monthly evolution:', error);
      return [];
    }
  },
};
